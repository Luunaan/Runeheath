...
/proc/parse_zone_fancy(zone, combat, combattarget, closeby, turnedaround, ontheground, grabbing, squinting, defiant, uncovered, has_penis, has_vagina, strength, self = FALSE)
	switch(zone)
		if(BODY_ZONE_PRECISE_R_HAND)
			if(closeby && !combat)
				if(squinting)
