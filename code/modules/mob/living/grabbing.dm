	...
	switch(user.used_intent.type)
		...
		if(/datum/intent/grab/shove)
			...
			if(!(M.mobility_flags & MOBILITY_STAND))
				...
				var/random_component = HAS_TRAIT(M, TRAIT_STOCKY) ? rand(-20, 5) : rand(-5, 5)
				if(prob(clamp((((4 + (((user.STASTR - M.STASTR)/2) + skill_diff)) * 10 + random_component) * combat_modifier), 5, 95)))
...
/obj/item/grabbing/bite/proc/bitelimb(mob/living/carbon/human/user) //implies limb_grabbed and sublimb are things
	...
	if(C.apply_damage(damage, BRUTE, limb_grabbed, armor_block))
		...
		else if (user.dna?.species?.venom_reagent && reagents)
