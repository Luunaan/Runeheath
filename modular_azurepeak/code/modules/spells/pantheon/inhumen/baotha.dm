...
/obj/effect/proc_holder/spell/invoked/painkiller/cast(list/targets, mob/living/user)
	if(isliving(targets[1]))
		...
		phy.pain_mult *= 0.5	//Literally halves your pain modifier.
		addtimer(VARSET_CALLBACK(phy, pain_mult, phy.pain_mult /= 0.5), 1 MINUTES)	//Adds back the 0.5 of pain, basically setting it back to 1.
